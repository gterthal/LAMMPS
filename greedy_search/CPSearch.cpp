...
m3_2_A3 = 1e30
P = 48.43106*atm2Pa*J2kcal*NA/m3_2_A3      # pressure in units of energy/volume [kcal/mol.A^3]

mvv2e = 2390.057364     # Da*A^2/fs^2 to kcal/mol
kB = 8.31451E-7         # in Da*A^2/(fs^2*K)
Pconv = 1.6388244954E+8 # Da/(A*fs^2) to atm

U_index = 7
V_index = 4
prop_index = 7    # sera usado ET para o calculo de CP a partir de H = ET + PV

# 0-Step |  1-T  | 2-P | 3-rho | 4-V | 5-Epot | 6-Ek | 7-ET | 8-H* | 9-Evdw | 10-Etail | 11-Elong 
#12-Ebond|13-Emol
...
for ind in range(K):
	...
	for (i,line) in enumerate(lines):
		...
		A_kn[ind,n[ind]] = float(elements[prop_index]) + P*float(elements[V_index])
...
MBAR = pymbar.mbar.MBAR(u_kln, n, verbose = True, relative_tolerance = 1.0e-10,
                        solver_protocol=({'method': 'Newton-CG'}, ), initialize = 'BAR')
...
npoints = 1000   # numero de pontos que serao formados a partir do tratamento dos dados
Tmin = min(temp)
Tmax = max(temp)
...
numpy.savetxt('resultH.txt', A_curve)
