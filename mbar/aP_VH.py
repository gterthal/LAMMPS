...
for ind in range(K):
	...
	for (i,line) in enumerate(lines):
		...
		A_kn[ind,n[ind]] = float(elements[prop_index])*float(elements[V_index]) + P*float(elements[V_index])*float(elements[V_index])
...
numpy.savetxt('resultVH.txt', A_curve)
numpy.savetxt('result2.txt', Var)
numpy.savetxt('result_sigmaVH.txt', dEA_k)
