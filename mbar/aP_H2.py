...
for ind in range(K):
	...
	for (i,line) in enumerate(lines):
		...
		A_kn[ind,n[ind]] = (float(elements[prop_index]) + P*float(elements[V_index]))*(float(elements[prop_index]) + P*float(elements[V_index]))
...
numpy.savetxt('resultH2.txt', A_curve)
numpy.savetxt('result_sigmaH2.txt', dEA_k)
